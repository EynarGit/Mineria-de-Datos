...
```{r}
ggplot(MuertesXDep2022, aes(x = Depreg , y= n)) + geom_bar(stat="identity", fill="red", alpha=.6, width=.4) + labs(x="Departamento",y= "Frecuencia") 
```
#Grafico de causas Principales de Muerte 2022

```{r}
ggplot(subset(CausasMuerte2022,n>1000)) + geom_point(aes(x = Caudef , y= n))
```
#Grafico de Edad de Muerte 2014

```{r}

ggplot(EdadMuerte2022 + geom_line(aes(x=Edadif,y=n)))
